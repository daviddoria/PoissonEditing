...
int main(int argc, char* argv[])
{
  ...
  if(argc < 5)
    {
    std::cout << "Usage: ImageToFill mask laplacian outputImage" << std::endl;
    exit(-1);
    }
  ...
  ImageReaderType::Pointer laplacianReader = ImageReaderType::New();
  laplacianReader->SetFileName(laplacianFilename);
  laplacianReader->Update();
  ...
  poissonFilter.SetLaplacian(laplacianReader->GetOutput());
